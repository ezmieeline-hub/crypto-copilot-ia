...
// ============================================================
// RENDER ANALYSE CLASSIQUE
// ============================================================
async function renderAnalysisResult(d) {
 $('analysisCard').classList.remove('hidden');
 $('tvResult').innerHTML = ''; // vider tradingview
 $('analysisTitle').textContent = `${d.summary.symbol} — ${d.trade.direction}`;
 ...
 $('compareBlock').innerHTML = `<div class="info-box"><p>${cmp.message}</p></div>`;
 }
 } catch (e) {
 $('compareBlock').innerHTML = '';
 }
 ...
 if (news) {
 const headlinesHtml = (news.headlines || []).map(n => `<p><a href="${n.url || '#'}" target="_blank">${n.title}</a> <em>(${n.source || '—'})</em></p>`).join('');
 const calendarHtml = (news.calendar || []).map(e => `<p>${e.title} — prévision : ${e.forecast ?? '—'} (précédent : ${e.previous ?? '—'})</p>`).join('');
 $('newsBlock').innerHTML = `
 ...
 if (mn) {
 renderMorningNote(mn, 'morningNoteBlock');
 } else {
 ...
 method: 'POST',
 body: JSON.stringify({taken: true, result_percent: isNaN(result_percent) ? null : result_percent, comment}),
 });
...
// RENDER TRADINGVIEW
// ============================================================
async function renderTradingViewResult(d) {
 $('analysisCard').classList.add('hidden'); // masquer carte classique
 ...
 const mn = d.morning_note;
 if (mn) {
 renderMorningNote(mn, 'tvResult', true);
 }
}

// ============================================================
// RENDER MORNING NOTE (utilitaire)
// ============================================================
function renderMorningNote(mn, targetId, append=false) {
 ...
 if (append) {
 $(targetId).innerHTML = $(targetId).innerHTML + html;
 } else {
...
async function loadHistory(){
 try{
  ...
  $('history').innerHTML=rows.map(r=>`<div class="history-row" style="cursor:pointer;" onclick="window.loadAnalysisById(${r.id})" title="Cliquer pour revoir l'analyse"><b>${r.symbol}</b><span>${r.signal}</span><span>${r.confidence}%</span><small>${r.created_at}</small></div>`).join('');
...
async function loadAnalysisById(id){
 ...
 try{
  ...
  // Détecter le format
  if (d.summary && d.trade) {
   await renderAnalysisResult(d);
  } else if (d.signal && d.symbol_detected) {
   await renderTradingViewResult(d);
  } else {
   message('Format d\'analyse inconnu.');
  }
...
$('clearHistoryBtn').onclick=async()=>{
 ...
 try{
  const r = await fetch('/api/history',{method:'DELETE',credentials:'include'});
  if(!r.ok) {
   const d = await r.json().catch(()=>({}));
...
async function loadDashboard(){
 ...
 <div class="stats" style="grid-template-columns:repeat(2,1fr) !important;">
 ...
 <div><span>📅 Analyses / jour</span><b>${d.avg_analyses_per_day}</b></div>
 </div>
 `;
...
$('tvBtn').onclick=async()=>{
 ...
 $('tvResult').innerHTML='<div class="info-box"><p>Analyse de la capture en cours...</p></div>';
 try{
...
$('morningNoteBtn').onclick = async () => {
 ...
 renderMorningNote(mn, 'morningNoteResult');
